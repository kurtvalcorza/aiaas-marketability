...
describe('DVI computation', () => {
  it('base and AD weight sets each sum to 1.0', () => {
    for (const overlay of ['basic', 'AD'] as const) {
      ...
      expect(w.costBarrier + w.technicalComplexity + w.localizationGap + w.uvpResonance).toBeCloseTo(1, 10);
    }
  });

  it('matches the docx weights', () => {
    expect(DVI_WEIGHTS.basic).toEqual({ costBarrier: 0.3, technicalComplexity: 0.25, localizationGap: 0.25, uvpResonance: 0.2 });
    expect(DVI_WEIGHTS.AD).toEqual({ costBarrier: 0.4, technicalComplexity: 0.1, localizationGap: 0.3, uvpResonance: 0.2 });
  });
  ...
  describe('computeDVI', () => {
    it('base worked example (C3/T5/L4/U5 -> 4.15)', () => {
      expect(computeDVI({ costBarrier: 3, technicalComplexity: 5, localizationGap: 4, uvpResonance: 5 })).toBe(4.15);
    });
    it('AD worked example (C5/T2/L4.5/U5 -> 4.55)', () => {
      expect(computeDVI({ costBarrier: 5, technicalComplexity: 2, localizationGap: 4.5, uvpResonance: 5 }, 'AD')).toBe(4.55);
    });
    it('all-zero ratings yield DVI 0 (now reachable); all-five yield 5', () => {
      const zero = { costBarrier: 0, technicalComplexity: 0, localizationGap: 0, uvpResonance: 0 };
      const five = { costBarrier: 5, technicalComplexity: 5, localizationGap: 5, uvpResonance: 5 };
